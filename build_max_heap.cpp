...
void heap(HeapType* h, int i) {
	int large = i;
	int l = 2 * i;
	int r = 2 * i + 1;

	if ((l < h->heap_size) && (h->heap[l].key > h->heap[large].key)) {
		large = l;
	}
	if ((r < h->heap_size) && (h->heap[r].key > h->heap[large].key)) {
		large = r;
	}
	if (large != i) {
		int temp;
		temp = h->heap[i].key;
		h->heap[i].key = h->heap[large].key;
		h->heap[large].key = temp;
		heap(h, large);
	}
}
...
void main()
{
	...
	int input_size = 100;	//10, 100, 1000
